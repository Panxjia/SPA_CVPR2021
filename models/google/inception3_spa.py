...
class Inception3(nn.Module):
    ...
    def forward(self, x, scg_flag=False):
        ...
        # 56 x 56 x 64
        sc_2 = sc_2_so = None
        if scg_flag and '2' in self.args.scg:
            sc_2, sc_2_so = self.hsc(feat1, fo_th=self.args.scg_fosc_th,
        ...
        # 28 x 28 x 192
        sc_3 = sc_3_so = None
        if scg_flag and '3' in self.args.scg:
            sc_3, sc_3_so = self.hsc(feat2, fo_th=self.args.scg_fosc_th,
                                            so_th=self.args.scg_sosc_th,
                                            order=self.args.scg_order)
        ...
        # 28 x 28 x 192
        sc_4 = sc_4_so = None
        if scg_flag and '4' in self.args.scg:
            sc_4, sc_4_so = self.hsc(feat3, fo_th=self.args.scg_fo_th,
                                            so_th=self.args.scg_so_th,
                                            order=self.args.scg_order)
        ...
        sc_5 = sc_5_so = None
        if scg_flag and '5' in self.args.scg:
            sc_5, sc_5_so = self.hsc(feat4, fo_th=self.args.scg_fosc_th,
                                            so_th=self.args.scg_sosc_th,
                                            order=self.args.nl_order)
        #Branch 1
        logits = self.inference(feat4)

        self.cam_map = logits

        return logits, (sc_2, sc_3, sc_4, sc_5), (sc_2_so, sc_3_so, sc_4_so, sc_5_so)
    ...
    def get_loss(self, logits, gt_labels, epoch=0, ram_start=10):
        gt = gt_labels.long()
        ...
        if self.args.ram and epoch >= ram_start:
            ra_loss = self.get_ra_loss(logits, gt, self.args.ram_th_bg, self.args.ram_bg_fg_gap)
            loss += self.args.ra_loss_weight * ra_loss
        else:
            ra_loss = torch.zeros_like(loss)
        return loss, ra_loss

    def get_ra_loss(self, logits, label, th_bg=0.3, bg_fg_gap=0.2):
        n, _, _, _ = logits.size()
